...
class DataTransformation:
    ...
    def get_data_transformer_object(cls)->Pipeline:
        ...
        ('sex_mode', SimpleImputer(strategy='most_frequent'), ['sex']),
        ('age_mean', SimpleImputer(strategy='mean'), ['age']),
        ('knn_imputer', KNNImputer(n_neighbors=3), selector(dtype_exclude='object')),
        ('onehot', OneHotEncoder(handle_unknown='ignore'), CATEGORICAL_COLUMN)
        ])     
            pipeline = Pipeline(steps=[('imputer_transformer', imputer_transformer)]) 
            return pipeline
        except Exception as e:
            raise ThyroidException(e, sys)
